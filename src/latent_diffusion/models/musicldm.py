...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        ...
        if k == 'text':
            ...
            return batch['fname']
        elif k in ['fbank', 'waveform', 'music_feat', 'sfx_feat']:
            x = batch[k].to(memory_format=torch.contiguous_format).float()
        
        elif k == 'music_feat':
            # 例如对音乐特征做一个 (B, T, F) -> (B, F, T) 转置
            if x.dim() == 3:
...
class MusicLDM(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        """
        Apply model with flexible conditioning that supports combining multiple conditioning keys.
        For example: both cross-attention and concatenation features.
        """

        if isinstance(cond, dict):
            # Already a dict: assume user provides keys like {"c_crossattn": [...], "c_concat": [...]}
            pass
        else:
            if not isinstance(cond, list):
                cond = [cond]

            # Example: you may have two separate features you want to combine
            # Here we assume the convention: cond = [crossattn_feature, concat_feature]
            if len(cond) == 2:
                cond = {
                    "c_crossattn": [cond[0]],
                    "c_concat": [cond[1]]
                ...
                # Fallback to single conditioning key
